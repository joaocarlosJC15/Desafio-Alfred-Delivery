...
describe('BooleanFieldValidation', () => {
  test('RequiredFieldValidation.validate não deve retornar nada se a propriedade fornecida for true', () => {
    const sut = makeSut()
    const error = sut.validate({ field: true })
    expect(error).toBeUndefined()
  })

  test('RequiredFieldValidation.validate não deve retornar nada se a propriedade fornecida for false', () => {
    const sut = makeSut()
  ...
  test('IntegerFieldValidation.validate deve retonar o erro "InvalidParamError" se a propriedade a ser testada for um numero', () => {
    const sut = makeSut()
